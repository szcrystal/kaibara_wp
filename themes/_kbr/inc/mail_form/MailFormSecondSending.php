...
//        }
    ...
    public function e_($array, $key, $num) { //num=1 : name/id 出力
	...
	public function checkRadioSession($argVal, $argName, $argFirst=0) { 
    	if($argFirst) {
        	...
        	if(isset($_SESSION[$argName]) && $_SESSION[$argName][1] == $argVal) {
                echo ' checked="checked"';
            }
        }
    ...
    public function sendMailAndSetDB() {
    	...
    	$mailTo = $this->adminRow->admin_email; //メインアドレス問い合わせの届け先（カンマ区切りで複数指定可）
    	$mainMail = $this->adminRow->admin_email; //確認メール内に記載される返信先（ヘッダーアドレス）
    
    	if($this->type == 'inspect') {
    		$subject = '視察募集がありました 〜'. $this->adminRow->admin_name; //get_bloginfo('name')　/* ★ */
            $return_subject = '視察募集を承りました ' . $this->adminRow->admin_name; /* ★ */
            $table_name = $wpdb->prefix . 'form_inspect'; /* ★ */
        }
        elseif($this->type == 'newshop') {
        	$subject = '出店者募集がありました 〜'. $this->adminRow->admin_name;
            $return_subject = '出店者募集を承りました ' . $this->adminRow->admin_name;
...
function addInfo($boolArg) {
    ...
    public function format_return($arg) {
        ...
        $adminHead = $this->adminRow->admin_head; //EOL内に入れるとエラーになる ObjectからのStringが良くないぽい
        //var_dump($adminHead);
...
// END to user mail sentence
        ...
        $context .= "\n\n\n\n\n";
        $context .= $this->adminRow->admin_foot;
        //$context .= "\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\t\n\t\n\t\n";
...
// END to user mail sentence
        ...
        //$context = $sentence;
        $context .= $this->format_mail_func($value="\n");
        $context .= "\n\n\n\n" . $this->adminRow->admin_foot;
        return $context;
    }



    public function setSessionAndCheckError() {
        //global $ar;
        //$ar = $this->getTitleAndName();
        
        foreach($this->ar as $key => $val) { //$ar : function.php global val
